 ...
 * 将service服务注册到RPCprovider中，以便可以处理来自客户端的RPC调用
 * RPC维护一个服务和他们的方法的注册表，使得当RPC调用到达时，可以查找并使用正确的服务和方法
 * @param service 传入需要注册的服务对象
 */
...
void RpcProvider::OnMessage(const muduo::net::TcpConnectionPtr &conn,
  ...
  // 从 m_serviceMap 获取 service 对象和 method 对象
  ...
  // 使用服务对象 service 创建请求 request 和响应 response 对象
  ...
  // 用户注册的service类 继承 .protoc生成的serviceRpc类 继承
  google::protobuf::Service
  // 用户注册的service类里面没有重写CallMethod方法，是 .protoc生成的serviceRpc类
  里面重写了google::protobuf::Service中 的纯虚函数CallMethod，而
  .protoc生成的serviceRpc类 会根据传入参数自动调取 生成的xx方法（如Login方法），
  由于xx方法被 用户注册的service类 重写了，因此这个方法运行的时候会调用
  用户注册的service类 的xx方法 真的是妙呀
  */
  // 真正调用方法
  service->CallMethod(method, nullptr, request, response, done);
}

// Closure的回调操作，用于 序列化 rpc 的响应和网络发送,发送响应回去
