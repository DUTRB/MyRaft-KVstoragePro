...
/**
 * ! @description: 接收leader发来的日志请求，主要用于检查当前日志是否匹配并同步leader的日志到本机
 *                 实现了Raft协议中Follower节点处理来自Leader节点的AppendEntries RPC请求的逻辑
...
/**
 * ! @description: 选举函数，实际发起选举，构造需要发送的rpc，并多线程调用sendRequestVote处理rpc及其相应。
 * @return {*}
 */
void Raft::doElection()
{
    ...
    if (m_status != Leader)
    {
        DPrintf("[  ticker-func-rf(%d)  ]  选举定时器到期 且不是 leader，开始选举!\n", m_me);
...
/**
 * ! @description: 向 follower 实际发送心跳,构造需要发送的rpc，并多线程调用sendRequestVote处理rpc及其相应
 * @return {*}
 */
void Raft::doHeartBeat()
{
    ...
    if (m_status == Leader)
    {
        DPrintf("[func-Raft::doHeartBeat()-Leader: {%d}] Leader的心跳定时器触发了且拿到 mutex，开始发送AE\n", m_me);
        ...
        for (int i = 0; i < m_peers.size(); i++)
        {
            ...
            std::thread t(&Raft::sendAppendEntries, this, i, appendEntriesArgs, appendEntriesReply,
                          appendNums); // 创建新线程并执行sendAppendEntries函数，并传递参数
            t.detach();
        }
        m_lastResetHearBeatTime = now(); // leader发送心跳，重置心跳时间表示以发送，就不是随机时间了
    }
}

/**
 * ? @description: 选举超时判断，当节点处于 folloewer 状态时，查看是否发起该选举，如果该发起选举则执行 doElection()
 * @return {*}
 */
void Raft::electionTimeOutTicker()
{
    while (true)
    {
        ...
        if (std::chrono::duration<double, std::milli>(suitableSleepTime).count() > 1)
        {
            ...
            // 使用ANSI控制序列将输出颜色修改为紫色   打印出设置的休眠时间和实际的休眠时间
        ...
        //      换句话说，由于选举超时计时器被重置了，当前的选举超时周期尚未结束，因此没有发生选举超时。
...
/**
 * @description: 负责发送快照的RPC，在发送完rpc后还需要负责接收并处理对端发送回来的响应
 * @param {int} server
 * @return {*}
 */
...
/**
 * ! @description: 接收别人发来的选举 RequestVote RPC 请求，主要检验是否要给对方投票。
...
void Raft::RequestVote(const raftRpcProctoc::RequestVoteArgs *args, raftRpcProctoc::RequestVoteReply *reply)
{
    ...
    myAssert(args->term() == m_currentTerm,
             format("[func--rf{%d}] 前面校验过 args.Term == rf.currentTerm，这里却不等", m_me));
    ...
    else
    {
        ...
        m_lastResetElectionTime = now();  // 认为必须要在投出票的时候才重置超时选举定时器，
                           ...
                           std::shared_ptr<raftRpcProctoc::RequestVoteReply> reply, std::shared_ptr<int> votedNum)
{
    ...
    // ? 调用 RequestVote 方法，向指定的服务器发送请求。ok 变量保存 RPC调用是否成功
    ...
    if (*votedNum >= m_peers.size() / 2 + 1)
    {
        ...
        *votedNum = 0; // 重置投票数
        if (m_status == Leader)
        {
            ...
            myAssert(false, format("[func-sendRequestVote-rf{%d}]  term:{%d} 同一个 term 当两次 Leader，error", m_me, m_currentTerm));
        }
...
/**
 * ! @description: 负责发送日志的RPC，在发送完rpc后还需要负责接收并处理对端发送回来的响应
 * @return {*}
 */
                             ...
                             std::shared_ptr<int> appendNums)
{
    ...
    {
        DPrintf("[func -sendAppendEntries  rf{%d}]  节点：{%d}的 term{%d} < rf{%d}的 term{%d}\n", m_me, server, reply->term(), m_me, m_currentTerm);
        return ok;
    }
    // 如果不是leader，不需要处理
    if (m_status != Leader)
    ...
    else // RPC 调用成功
    {
        *appendNums = *appendNums + 1; // 成功返回的节点加一
        DPrintf("-----tmp------- 节点{%d}返回 true,当前*appendNums{%d}", server, *appendNums);
...
// 服务器的端口是peers[me]。所有服务器的peer[]阵列
// 有相同的顺序。persister是该服务器保存持久状态的位置
// applyCh是一个通道
// 测试人员或服务希望Raft发送ApplyMsg消息。

/**
 * ! @description: peers[me] : 服务器端口  persister : 持久化  applyCh : 测试人员或服务希望在Raft上发送Applymsg消息的通道
 * @return {*}
 */
void Raft::init(std::vector<std::shared_ptr<RaftRpcUtil>> peers, int me, std::shared_ptr<Persister> persister,
                std::shared_ptr<LockQueue<ApplyMsg>> applyCh)
{
    m_peers = peers;         // 与其他结点沟通的 RaftRpcUtil rpc类
