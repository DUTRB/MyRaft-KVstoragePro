...
void KvServer::DprintfKVDB() {
  ...
  DEFER {
    ...
    //     DPrintf("[DBInfo ----]Key : %s, Value : %s", &item.first,
    //     &item.second);
    // }
    m_skipList.print_list();
  };
}
...
void KvServer::ExecuteAppendOpOnKVDB(Op op) {
  ...
  //    DPrintf("[KVServerExeAPPEND-----]ClientId :%d ,RequestID :%d ,Key : %v,
...
void KvServer::ExecuteGetOpOnKVDB(Op op, std::string *value, bool *exist) {
  ...
  *exist = false;
  ...
  if (*exist) {
    //                DPrintf("[KVServerExeGET----]ClientId :%d ,RequestID :%d
    //                ,Key : %v, value :%v", op.ClientId, op.RequestId, op.Key,
    //                value)
  } else {
    //        DPrintf("[KVServerExeGET----]ClientId :%d ,RequestID :%d ,Key :
    //        %v, But No KEY!!!!", op.ClientId, op.RequestId, op.Key)
  }
  DprintfKVDB();
}
...
void KvServer::ReadSnapShotToInstall(std::string snapshot) {
  ...
  //
  //    if d.Decode(&persist_kvdb) != nil || d.Decode(&persist_lastRequestId) !=
  //    nil {
  //                DPrintf("KVSERVER %d read persister got a
  //                problem!!!!!!!!!!",kv.me)
  //        } else {
...
KvServer::KvServer(int me, int maxraftstate, std::string nodeInforFileName,
  ...
  std::cout << "raftServer node: " << m_me
            << "-- start to sleep to wait all other raftnode start !"
            << std::endl;
  sleep(6);
  std::cout << "raftServer node: " << m_me
  ...
  m_lastSnapShotRaftLogIndex =
      0; // todo:感觉这个函数没什么用，不如直接调用raft节点中的snapshot值？？？
