...
private:
    ...
    std::vector<raftRpcProctoc::LogEntry> m_logs; // 日志条目数组，包含了状态机要执行的指令集，以及收到领导时的任期号
                                                  // 这两个状态所有结点都在维护，易失
    int m_commitIndex; // 记录当前节点可以提交的日志条目的索引
...
public:
    ...
    void leaderUpdateCommitIndex();           // leader 更新 commitIndex
    bool matchLog(int logIndex, int logTerm); // 对应Index的日志是否匹配，只需要Index和Term就可以知道是否匹配
    void persist();                           // 持久化

    // 变成 candidate 之后需要让其他结点给自己投票
    ...
    void pushMsgToKvServer(ApplyMsg msg); // 给上层的 kvserver 层发送消息
